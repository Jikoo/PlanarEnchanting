...
class AnvilTest {
  ...
  void beforeAll() throws ClassNotFoundException {
    ...
    tool = ItemType.DIAMOND_PICKAXE;

    // Force RepairMaterial to fall through to pre-baked defaults.
    doThrow(IncompatibleClassChangeError.class).when(tool).getDefaultData(any());
