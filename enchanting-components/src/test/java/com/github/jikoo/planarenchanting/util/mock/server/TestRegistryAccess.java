...
public class TestRegistryAccess implements RegistryAccess {
  ...
  public <T extends @NotNull Keyed> @NotNull Registry<T> getRegistry(@NotNull RegistryKey<T> registryKey) {
    return (Registry<T>) REGISTERS.computeIfAbsent(registryKey, regKey -> {
      ...
      doAnswer(invocation -> {
        TypedKey<?> typedKey = invocation.getArgument(0);
        return registry.get(typedKey.key());
      }).when(registry).get(any(TypedKey.class));
      doAnswer(invocation -> {
        TypedKey<T> typedKey = invocation.getArgument(0);
        return registry.getOrThrow(typedKey.key());
      }).when(registry).getOrThrow(any(TypedKey.class));
      ...
      doAnswer(invocation -> {
        try {
          registry.getTag(invocation.getArgument(0));
          return true;
        } catch (NoSuchElementException e) {
          return false;
        }
      }).when(registry).hasTag(any(TagKey.class));
